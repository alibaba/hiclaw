...
- **Use `copaw channels send` via shell** to send a follow-up to that room:
  ...
  copaw channels send \
    ...
    --text "@{worker}:${HICLAW_MATRIX_DOMAIN} Have you started task {task-id}? If you are already working, send a short progress update. If you are blocked, say what you need."
  ```
